...
전통적으로 AI 분야에서는 'Scaling Laws'라는 개념이 지배적이었었다. 더 큰 모델, 더 많은 데이터, 더 강력한 컴퓨팅을 투자하면 성과가 자연스럽게 향상될 것이라는 예측이었다[1]. 하지만 최근 연구들은 이 법칙이 만능일 뿐임을 보여주고 있다. 스케일링에 따른 체감 성능의 한계가 명확히 드러나고 있다.
